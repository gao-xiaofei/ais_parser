...
/*! \file
   ...
   Reserved \version 1.0
    ...
    Includes information like wind information, water level and flow, lock
   order, lock time.
...
*/
/* ----------------------------------------------------------------------- */
int parse_seaway1_1(sixbit *state, seaway1_1 *result) {
  int length;
  int i;
  int j;
  ...
  if ((length < 0) || (length > 1008)) return 2;

  /* Clear out the structure first */
  memset(result, 0, sizeof(seaway1_1));

  for (i = 0; i < 4; i++) {
    if (get_timetag(state, &result->report[i].utc_time)) return 3;
    ...
    if (sixbit_length(state) < 192) break;
  }
  return 0;
}
...
*/
/* ----------------------------------------------------------------------- */
int parse_seaway1_2(sixbit *state, seaway1_2 *result) {
  int length;
  int i;
  int j;
  ...
  for (i = 0; i < 6; i++) {
    ...
    /* Is there enough data for another? */
    if (sixbit_length(state) < 144) break;
  }
  return 0;
}
...
*/
/* ----------------------------------------------------------------------- */
int parse_seaway1_3(sixbit *state, seaway1_3 *result) {
  int length;
  int i;
  int j;
...
*/
/* ----------------------------------------------------------------------- */
int parse_seaway1_6(sixbit *state, seaway1_6 *result) {
  int length;
  int i;
  int j;
...
*/
/* ----------------------------------------------------------------------- */
int parse_seaway2_1(sixbit *state, seaway2_1 *result) {
  int length;
  int i;
  int j;
  ...
  if (get_timetag(state, &result->utc_time)) return 3;
  ...
  for (i = 0; i < 6; i++) {
    ...
    if (get_timetag(state, &result->schedule[i].eta)) return 4;
    result->schedule[i].spare = (int)get_6bit(state, 9);

    /* Is there enough data for another? */
    if (sixbit_length(state) < 120) break;
  }
  return 0;
}
...
*/
/* ----------------------------------------------------------------------- */
int parse_seaway2_2(sixbit *state, seaway2_2 *result) {
  int length;
  int j;
  ...
  if (get_timetag(state, &result->last_ata)) return 3;
  ...
  if (get_timetag(state, &result->first_eta)) return 3;
  ...
  if (get_timetag(state, &result->second_eta)) return 3;
...
*/
/* ----------------------------------------------------------------------- */
int parse_seaway32_1(sixbit *state, seaway32_1 *result) {
  int length;
...
*/
/* ----------------------------------------------------------------------- */
int parse_pawss1_4(sixbit *state, pawss1_4 *result) {
  int length;
  int i;
  int j;
...
*/
/* ----------------------------------------------------------------------- */
int parse_pawss1_5(sixbit *state, pawss1_5 *result) {
  int length;
  int i;
  int j;
...
/** PAWSS Message 2.3 - Vessel Procession Order Message
    ...
    This function parses a Vessel Procession Order Message into seaway1_1
   structure
*/
/* ----------------------------------------------------------------------- */
int parse_pawss2_3(sixbit *state, pawss2_3 *result) {
  int length;
  int i;
  int j;
  ...
  for (i = 0; i < 4; i++) {
    ...
    /* Is there enough data for another? */
    if (sixbit_length(state) < 184) break;
  }
  return 0;
}
