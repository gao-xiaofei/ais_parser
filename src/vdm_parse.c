...
/*! \file
    ...
    \author Copyright 2006-2008 by Brian C. Lane <bcl@brianlane.com>, All Rights
   Reserved \version 1.8
...
*/
/* ----------------------------------------------------------------------- */
int get_timetag(sixbit *state, timetag *datetime) {
  int length;
...
*/
/* ----------------------------------------------------------------------- */
void conv_sign(unsigned int sign_bit, int *value) {
  if (*value & sign_bit) {
...
*/
/* ----------------------------------------------------------------------- */
int pos2ddd(long latitude, long longitude, double *lat_dd, double *long_ddd) {
...
*/
/* ----------------------------------------------------------------------- */
int pos2dmm(long latitude, long longitude, short *lat_dd, double *lat_min,
...
*/
/* ----------------------------------------------------------------------- */
int conv_pos(long *latitude, long *longitude) {
  /* LATITUDE */
...
/* ----------------------------------------------------------------------- */
/** Convert unsigned 1/10 minute position to signed (as used in Type 27
   messages)
...
*/
/* ----------------------------------------------------------------------- */
int conv_pos27(long *latitude, long *longitude) {
  /* LATITUDE */
  /* Convert latitude to signed number */
  if (*latitude & 0x10000) {
...
*/
/* ----------------------------------------------------------------------- */
int assemble_vdm(ais_state *state, char *str) {
  unsigned int total;
  unsigned int num;
  ...
  /* Is the string an AIS message? Allow any start character and any
     device pair.
  */
  if ((p = find_nmea_start(str)) == NULL) return 3;
  if ((strncmp(p + 3, "VDM", 3) != 0) && (strncmp(p + 3, "VDO", 3) != 0)) {
    /* Not an AIS message */
    return 3;
  }
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_1(ais_state *state, aismsg_1 *result) {
  if (!state) return 1;
  if (!result) return 1;
  ...
  result->_true = (int)get_6bit(&state->six_state, 9);
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_2(ais_state *state, aismsg_2 *result) {
  int length;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_3(ais_state *state, aismsg_3 *result) {
  if (!state) return 1;
  if (!result) return 1;

  if (strlen(state->six_state.bits) != 28) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_4(ais_state *state, aismsg_4 *result) {
  if (!state) return 1;
  if (!result) return 1;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_5(ais_state *state, aismsg_5 *result) {
  unsigned int i;

  if (!state) return 1;
  if (!result) return 1;

  if (strlen(state->six_state.bits) != 71) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_6(ais_state *state, aismsg_6 *result) {
  int length;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length < 88) || (length > 1008)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_7(ais_state *state, aismsg_7 *result) {
  int length;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length < 72) || (length > 168)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_8(ais_state *state, aismsg_8 *result) {
  int length;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length < 56) || (length > 1008)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_9(ais_state *state, aismsg_9 *result) {
  if (!state) return 1;
  if (!result) return 1;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_10(ais_state *state, aismsg_10 *result) {
  if (!state) return 1;
  if (!result) return 1;

  if (strlen(state->six_state.bits) != 12) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_11(ais_state *state, aismsg_11 *result) {
  if (!state) return 1;
  if (!result) return 1;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_12(ais_state *state, aismsg_12 *result) {
  int length;
  int i;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length < 72) || (length > 1008)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_13(ais_state *state, aismsg_13 *result) {
  int length;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_14(ais_state *state, aismsg_14 *result) {
  int length;
  int i;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length < 40) || (length > 1008)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_15(ais_state *state, aismsg_15 *result) {
  int length;
  ...
  /* Check the length of the packet */
  length = strlen(state->six_state.bits) * 6;
  if ((length < 88) || (length > 162)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_16(ais_state *state, aismsg_16 *result) {
  int length;
  ...
  /* Check the length of the packet */
  length = strlen(state->six_state.bits) * 6;
  if ((length != 96) && (length != 144)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_17(ais_state *state, aismsg_17 *result) {
  int length;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length < 80) || (length > 816)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_18(ais_state *state, aismsg_18 *result) {
  if (!state) return 1;
  if (!result) return 1;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_19(ais_state *state, aismsg_19 *result) {
  int i;

  if (!state) return 1;
  if (!result) return 1;

  if (strlen(state->six_state.bits) != 52) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_20(ais_state *state, aismsg_20 *result) {
  int length;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length < 72) || (length > 162)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_21(ais_state *state, aismsg_21 *result) {
  int length;
  int i;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length < 272) || (length > 360)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_22(ais_state *state, aismsg_22 *result) {
  if (!state) return 1;
  if (!result) return 1;
  ...
  if (result->addressed) {
    ...
    result->addressed_1 =
        (result->NE_longitude << 12) + (result->NE_latitude >> 5);
    result->addressed_2 =
        (result->SW_longitude << 12) + (result->SW_latitude >> 5);
  } else {
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_23(ais_state *state, aismsg_23 *result) {
  if (!state) return 1;
  if (!result) return 1;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_24(ais_state *state, aismsg_24 *result) {
  int length;
  int i;
  ...
  length = strlen(state->six_state.bits) * 6;
  if ((length != 162) && (length != 168)) return 2;
...
*/
/* ----------------------------------------------------------------------- */
int parse_ais_27(ais_state *state, aismsg_27 *result) {
  if (!state) return 1;
  if (!result) return 1;
  int length;
  // Length according to ITU-1374 is 96 bits. However, in the wild these are
  // sometimes transmitted with 168 bits (a full slot). Robust decoders should
  // warn when this occurs but decode the first 96 bits.
  length = strlen(state->six_state.bits) * 6;
  if ((length < 96) || (length > 168)) return 2;
