...
typedef struct {
  ...
  char wind_avg;   //!< 7 bits   : Average of wind speed for the last 10 minutes
                   //!< 0-120kts
  char wind_gust;  //!< 7 bits   : Maximum wind speed during the last 10 minutes
                   //!< 0-120kt
  ...
  char
      speed_2;  //!< 8 bits   : Current measured at chosen level 0.0 to 25.0 kts
  int dir_2;    //!< 9 bits   : Current direction 0-359 degrees
  char level_2;  //!< 5 bits   : Measuring level 0-30m
  char
      speed_3;  //!< 8 bits   : Current measured at chosen level 0.0 to 25.0 kts
...
typedef struct {
  ...
  char
      units;  //!< 2 bits   : 0=not avail. 1=kg 2=tons (1E3kg) 3=1000tons(1E6kg)
...
typedef struct {
  ...
  int extension;     //!< 10 bits : Extension of closed area radius (0-1000
                     //!< 1001=not available)
...
typedef struct {
  ...
  char to_hour;      //!< 5 bits   : 0-23 24=not available
  char to_minute;    //!< 6 bits   : 0-59 60=not available
  int current_dir;   //!< 9 bits   : Current direction in degrees 0-359 360=not
                     //!< available
  char
      current_speed;  //!< 7 bits   : Current speed 0-12.6 kts 127=not available
} tidal_window;
 ...
 */
typedef struct {
  int num_persons;  //!< 13 bits  : Number of Persons on Board 0-8191 0=not
                    //!< available
  char spare;       //!< 3 bits   : spare
} imo1_16;
...
typedef struct {
  ...
  unsigned char
      sog;  //!< 8 bits   : Speed over ground 0-254kts 255=not available
} pseudo_target;
...
int parse_imo1_11(sixbit *state, imo1_11 *result);
int parse_imo1_12(sixbit *state, imo1_12 *result);
int parse_imo1_13(sixbit *state, imo1_13 *result);
int parse_imo1_14(sixbit *state, imo1_14 *result);
int parse_imo1_15(sixbit *state, imo1_15 *result);
int parse_imo1_16(sixbit *state, imo1_16 *result);
int parse_imo1_17(sixbit *state, imo1_17 *result);
